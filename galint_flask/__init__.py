...
def _background_services_enabled() -> bool:
    value = os.environ.get("GALINT_DISABLE_BACKGROUND_SERVICES", "false")
...
# Configurar Poppler antes de qualquer import de pdf2image
try:
    from .poppler_config import configure_poppler_path
...
def create_app(config_name: str | None = None) -> Flask:
    ...
    try:
        if _background_services_enabled() and os.environ.get("GALINT_TELEGRAM_POLLING", "true").strip().lower() in ("1", "true", "yes"):
    ...
    try:
        if _background_services_enabled() and _should_run_startup_side_effects() and os.environ.get(
...
def _initialize_scheduler(app: Flask) -> None:
    ...
    try:
        if not _background_services_enabled():
            app.logger.info("Serviços em background desabilitados por GALINT_DISABLE_BACKGROUND_SERVICES=true")
            return
